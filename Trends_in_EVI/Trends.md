...
In this paper, they consider a continuously changing extreme value index and try to estimate the functional extreme value index accurately.
...
The goal of this paper is to estimate the function $\gamma$ and test the hypothesis that $\gamma=\gamma_0$ for some given function $\gamma_0$. In particular, it can be applied to test whether the extreme value index remains at a constant level across time.

And this is the model setting and do you have any questions about this part?  
...
Then there will be [2nh] observations in the h-neighborhood for each $s\in [h,1-h]$. 

---

To apply any known estimators for extreme value index, such as the Hill estimator, we choose an intermediate sequence $k=k(n)$ such that as $n \to \infty, k \to \infty$ and $k/n\to 0$. This choice of $k$ is very standard in the extreme value analysis.

Then, we will use the top [2kh] order statistics among the [2nh] local observation in the h-neighborhood of $s$ to estimate $\gamma(s)$. And the mathematical definition of the local Hill estimator is as this one. Aparting from using the local data, the local Hill estimator is the same as the standard Hill estimator in the iid case.
...
Condition (5) is the one usually required for extreme value analysis to guarantee to have no asymptotic bias in the estimator.  In the iid case, we often assume that $\sqrt{k_n} A(n/k_n)$ tends to 0. Here, we require a uniform form so condition (5) takes supremum of s over [0,1].


Condition (6) indeed puts a restriction on the smooth the function $\gamma$.  $k_n$ cannot be too large.

Condition (7) requires that the U function are sufficently smooth. Recall that the U function is related to the quantile function. So condition (7) indeed requires that the quantile functions are sufficiently smooth in the tail in the short interval.
...
In addition, the local estimators converges with a slow speed of convergence $1/\sqrt{2kh}$. To achieve the hypothesis testing goal, consider the estimation of captial $\Gamma(s)$, which is the integrate of $\gamma$ function from 0 to s. captial $\Gamma=\Gamma_0$ is the  equivalent hypothesis.

The function captial $\Gamma$ is estimated by aggregating the local estimators of $\gamma(s)$ to a global estimator. Consider a discretized version of the local Hill estimator. And then define the estimator of captial $\Gamma(s)$ as the integral of the discretized version of local Hill estimator.
...
In the first panel, we observe that under the null hypothesis, the rejection rates, that is the type1 error, are close to the significance level. When $h=0.025$, The difference  between the two choice of $k$ is very samll, they both perform very well. When $h=0.04$, the choice for $k=200$ is better and when $k=100$, the rejection rate is less than the significance level.
...
---
 ...
 The obtained p-value against various levels of $k$ are ploted in the upper panel in this figure. We see that the p-value is always great than 0.05. So  the conclusion is that do not reject the null hypothesis under the 5% significance level. Then estimate the extreme value index by appling Hill estimator to all obeservtaions, that is, estimating capital $\Gamma(1)$.
...
The obtained p-alues againist varios level of $k$ are shown in this figure. The upper one is for period 1. The lower one is for period 2. For the period 1, from 1988 to 2012, the null hypothesis is not rejected for k up to 750 under the 0.05 significan level. And we can think the extreme value indices remain constant over this period.
 And for the lower figure, we see that the p value is less than 0.05 for almost all the choice of k.  we reject the constant extreme value index hypotheis for period 2. And  conclude that there is a change of extreme value index during the period from 1963 to 2012.
...
The authors split the dataset of period 2 into two subsets that consist of daily returns on the even and odds days, respectively. The split of the full data helps to reduce the serial dependece and data from each subset is more close to the iid assumption.

We observe that for the even days, 
the null hypothesis is not rejected at 5% sigificance level, 
For the daily returns on the odd days, the result is not conclusive : for k ranging from 400 to 600, the null hypothesis is not rejected at 5% siginificance level.
